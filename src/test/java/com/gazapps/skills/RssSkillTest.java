...
/**
 * Unit tests for {@link RssSkill}.
 *
 * <p>All tests here are pure unit tests — no network calls, no Playwright.
...
class RssSkillTest {
    ...
    // ── parseItems — XML parsing (RSS 2.0) ────────────────────────────────────
    ...
    void parseItems_validRss2_returnsItems() throws Exception {
        ...
        List<RssSkill.FeedItem> items = RssSkill.parseItems(xml, 10);
    ...
    void parseItems_maxItemsRespected() throws Exception {
        byte[] xml = RSS2_SAMPLE.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        List<RssSkill.FeedItem> items = RssSkill.parseItems(xml, 1);
    ...
    void parseItems_maxItemsZero_returnsEmpty() throws Exception {
        byte[] xml = RSS2_SAMPLE.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        List<RssSkill.FeedItem> items = RssSkill.parseItems(xml, 0);
        assertEquals(0, items.size());
    }

    // ── parseItems — XML parsing (Atom 1.0) ──────────────────────────────────
    ...
    // ── parseItems — HTML stripping in descriptions ───────────────────────────
    ...
    // ── parseItems — description truncation ──────────────────────────────────
    ...
    // ── searchInFeed — keyword filtering ─────────────────────────────────────

    @Test
    void searchInFeed_keywordInTitle_isFound() throws Exception {
        // Uses package-private parseItems via same class — test the filtering logic
        byte[] xml = RSS2_SAMPLE.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        List<RssSkill.FeedItem> all = RssSkill.parseItems(xml, 10);

        String keyword = "first";
        List<RssSkill.FeedItem> matches = all.stream()
